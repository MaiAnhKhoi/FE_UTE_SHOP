    ...
    const onSubmit = async (e: FormEvent) => {
        ...
        try {
            ...
            if (mode === "signup") {
                ...
                // await api.post("/auth/forgot-password/verify", { email, otp, password });
                await api.post("/auth/forgot-password/verify", { email, otp, password });
            }
            ...
            setErrors({
                form:
                    ...
                    ax.message ||
                    "Verification failed. Please check and try again.",
            });
        } finally {
            setLoading(false);
        }
        ...
        try {
            ...
            setErrors({
                form:
                    ax.response?.data?.message ||
                    ax.message ||
                    "Could not resend OTP. Please try later.",
            });
        } finally {
            setLoading(false);
        }
            ...
            <div className="w-[400px] max-w-full rounded-lg p-8 text-center border border-white/50 bg-white/10 backdrop-blur-[9px]">
                <form className="flex flex-col" onSubmit={onSubmit} noValidate>
                    ...
                    <Button type="submit" loading={loading} className="mt-2 w-full">
                        {loading ? "Processing..." : mode === "signup" ? "Verify & Activate" : "Reset password"}
                    </Button>
